angular.module("app").service("fisModal", fisModal);
...
function fisModal($q, $uibModal, fisConfig, api, invoiceFactory) {
  ...
  function confirmOrCancel(data) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/confirm-or-cancel-modal/confirm-or-cancel-modal.template.html",
  ...
  function creditNoteExternalInvoice() {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/credit-note-external-invoice-modal/credit-note-external-invoice-modal.template.html",
  ...
  function creditNoteItemModal(modalData) {
    return $uibModal.open({
      templateUrl:
        "app/modals/credit-note-item-modal/credit-note-item-modal.template.html",
  ...
  function creditNoteTurnoverUsedModal(modalData) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/credit-note-turnover-used-modal/credit-note-turnover-used-modal.template.html",
  ...
  function duePaymentNotificationModal(data) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/due-payment-notification-modal/due-payment-notification-modal.template.html",
  ...
  function invoiceBuyerSelectModal(invoice) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/invoice-buyer-select-modal/invoice-buyer-select-modal.template.html",
  ...
  function invoiceItemEdit(invoice, itemIndex) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/invoice-item-edit-modal/invoice-item-edit-modal.template.html",
  ...
  function invoiceItemTemplatePicker() {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/invoice-item-template-picker-modal/invoice-item-template-picker-modal.template.html",
  ...
  function invoicePriceStructure(invoice) {
    return $uibModal.open({
      templateUrl:
        "app/modals/invoice-price-structure-modal/invoice-price-structure-modal.template.html",
  ...
  function invoiceScheduleModal(invoice) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/invoice-schedule-modal/invoice-schedule-modal.template.html",
  ...
  function numericInput(initialData) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/numeric-input-modal/numeric-input-modal.template.html",
  ...
  function buyerCreateModal() {
    return $uibModal.open({
      templateUrl:
        "app/modals/komitent-unos-modal/komitent-unos-modal.template.html",
  ...
  function buyerUpdateModal(buyerId) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/komitent-izmjena-modal/komitent-izmjena-modal.template.html",
  ...
  function ordersFinalPaymentMethod(orders, total) {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/order-payment-modal/order-payment-modal.template.html",
      controller: "OrderPaymentModalController",
      controllerAs: "ctrl",
      size: "lg",
      backdrop: "static",
      windowTemplateUrl: "app/modals/window-fixed.template.html",
  ...
  function addPaymentMethods(
    ...
    updated_invoice,
    advance_invoice,
    corrective_for_advance_invoice
  ) {
    let settings = {};
    ...
    paymentMethodPicker(settings).then(function (result) {
      ...
      if (advance_invoice) {
        ...
        advance_payment_method.amount = new Big(
          corrective_for_advance_invoice.ukupna_cijena_prodajna
        )
          .times(-1)
          .toNumber();
      ...
      for (let ii = 0; ii < result.items.length; ii++) {
        modal_payment_methods.push(
          invoiceFactory.createPaymentMethod(result.items[ii].id)
        );
  ...
  function distributeDifferenceEvenly(
    invoice,
    updated_invoice,
    advance_invoice
  ) {
    ...
    paymentMethodPicker(settings).then(function (result) {
      ...
      let modal_payment_methods = [];
      let parts = invoiceFactory.splitAmount(
        invoice.payment_methods_total_difference,
        result.items.length
      );
      for (let ii = 0; ii < result.items.length; ii++) {
        let payment_method = invoiceFactory.createPaymentMethod(
          result.items[ii].id
        );
      ...
      modal_payment_methods = invoiceFactory.mergePaymentMethods(
        invoice.payment_methods,
        modal_payment_methods
      );

      let bodyText =
        '<div class="mb-3">Načini plaćanja će biti izmijenjeni na sledeći način:</div>';
      for (let ii = 0; ii < modal_payment_methods.length; ii++) {
        ...
        bodyText +=
          '<div class="dash-info bg-white">\n' +
          '            <div class="dash-info-start"><span>' +
          label +
          "</span></div>\n" +
          '            <div class="dash-info-spacing"></div>\n' +
          '            <div class="dash-info-end"><span>' +
          value +
          "</span></div>\n" +
          "        </div>";
      }
      ...
      service
        ...
        .then(function (result) {
          if (result.isConfirmed) {
            invoice.payment_methods = modal_payment_methods;
            invoice.is_cash =
              modal_payment_methods[0].payment_method_type.is_cash;
            invoiceFactory.recalculatePaymentMethodTotals(invoice);
          }
        });
    });
  ...
  function distributeTotalEvenly(
    invoice,
    updated_invoice,
    advance_invoice,
    corrective_for_advance_invoice
  ...
  function depositNeededModal() {
    return service.confirmOrCancel({
      ...
      bodyText:
        "Za rad sa gotovinom morate definisati iznos dnevnog depozita. Da li želite da definišete iznos depozita sada?",
  ...
  function invalidForm() {
    return service.confirm({
      ...
      headerText: "Greška",
      bodyText:
        "Podaci nisu ispravno popunjeni. Ispravite greške pa pokušajte ponovo.",
    });
  }
  ...
  function certificateUpload() {
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/certificate-upload-modal/certificate-upload-modal.template.html",
  ...
  function paymentMethodPicker(initialData) {
    ...
    initialData.isMultiselect =
      initialData.isMultiselect !== undefined
        ? initialData.isMultiselect
        : false;
    initialData.selectedItems =
      initialData.selectedItems !== undefined
        ? angular.copy(initialData.selectedItems)
        : [];
    initialData.compare = (a, b) => {
      return a.id === b.id;
    };
    ...
    if (
      initialData.showCashTab === undefined ||
      initialData.showCashTab === null
    ) {
    ...
    if (
      initialData.showNonCashTab === undefined ||
      initialData.showNonCashTab === null
    ) {
    ...
    let modalInstance = $uibModal.open({
      templateUrl:
        "app/modals/payment-method-select-modal/payment-method-select-modal.template.html",
