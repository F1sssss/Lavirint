...
function FakturaUnosPoGrupamaController(
  ...
  ctrl.artikalGrid = createGrid([], 3, 3, "unosPoGrupama.artikalGrid");
  ...
  function updateOrCreateItemFromItemTempate(itemTemplate, quantity) {
    if (itemTemplate.invoiceItem === null) {
      itemTemplate.invoiceItem = invoiceFactory.addItemFromItemTemplate(
        ctrl.racun,
        itemTemplate
      );
    }
  ...
  function selectItemGroup(grupa_artikala) {
    fisGui.wrapInLoader(function () {
      return api.grupaArtikala.poId.zaliha
        .listaj(grupa_artikala.id)
        .then(function (data) {
          ...
          for (let ii = 0; ii < ctrl.zalihe.length; ii++) {
            ...
            for (let jj = 0; jj < ctrl.racun.stavke.length; jj++) {
              if (
                ctrl.zalihe[ii].artikal_id ===
                ctrl.racun.stavke[jj].magacin_zaliha.artikal.id
              ) {
                ctrl.zalihe[ii].invoiceQuantity =
                  ctrl.racun.stavke[jj].kolicina;
  ...
  function updateGridPage(grid, page) {
    ...
    for (let ii = 0; ii < grid.rowNum; ii++) {
      ...
      let end =
        grid.currentPageIndex * grid.itemsPerPage +
        (ii * grid.colNum + grid.colNum);
  ...
  function editQuantityInNumericModal(itemTemplate) {
    fisModal
      .numericInput({ value: itemTemplate.invoiceQuantity })
  ...
  function azurirajStavku(index) {
    let cachedInvoiceTemplateId =
      ctrl.racun.stavke[index].magacin_zaliha.artikal_id;
  ...
  function sendData(paymentMethodTypeId) {
    ...
    fisGui
      .wrapInLoader(function () {
        ...
        return api[api_name](podaci).then(function (data) {
          return data;
        });
      })
      .then(function (data) {
