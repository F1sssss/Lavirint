angular
    .module('app')
    .controller('FakturaPregledPorudzbineController', FakturaPregledPorudzbineController);
...
function FakturaPregledPorudzbineController(
    ...
    ctrl.selectedOrdersAll = ctrl.stranica.stavke.some(racun => ctrl.selectedOrders[racun.efi_broj_fakture]);
    ...
    function countSelectedOnPage(){
        return ctrl.stranica.stavke.filter((racun) => ctrl.selectedOrders[racun.efi_broj_fakture]).length;
    }

    function toggleSelectAllOnPage() {
        const selectOn = !Boolean(ctrl.countSelectedOnPage());

        ctrl.stranica.stavke.forEach(racun => ctrl.selectedOrders[racun.efi_broj_fakture] = selectOn);
    }


    function mousedownInvoice(efi_broj_fakture){
        ctrl.longTouchTimeout = setTimeout(() => {
            ctrl.selectedOrders[efi_broj_fakture] = !ctrl.selectedOrders[efi_broj_fakture];
        }, 200);
    }
