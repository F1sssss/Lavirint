...
function run(
  ...
  $rootScope.selectedOrdersTotals = {};
  ...
  function setCertificateExpirationDate(value) {
    ...
    if ($rootScope.certificate_expiration_date === null) {
      ...
      const diff =
        new Date($rootScope.certificate_expiration_date).getTime() -
        new Date().getTime();
      if (diff <= 0) {
  ...
  function goToHomepage() {
    if (
      fisConfig.user.podesavanja_aplikacije.pocetna_stranica ===
      "/prodaja/racun/unos"
    ) {
  ...
  function transitionsOnFinish(transition) {
    ...
    $rootScope.showDesktopTitle =
      to.showDesktopTitle !== undefined ? to.showDesktopTitle : false;
        ...
        if (!data.danasnji_depozit || data.danasnji_depozit.status !== 2) {
          return fisModal.depositNeededModal().then(function (result) {
            if (result.isConfirmed) {
              return transition.router.stateService.target("depozit", {
                "#": "depozit-iznos",
              });
            }
            ...
            if (
              from.url === "^" &&
              fisConfig.user.podesavanja_aplikacije.pocetna_stranica === to.url
            ) {
              return transition.router.stateService.target(
                "faktura_pregled_redovnih",
                {
