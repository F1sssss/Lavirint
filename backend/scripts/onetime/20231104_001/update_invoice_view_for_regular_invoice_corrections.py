...
total_rows = db.session.query(Faktura) \
    ...
    .filter(Faktura.korigovana_faktura.has(Faktura.tip_fakture_id == Faktura.TYPE_REGULAR)) \
    .count()
...
for ii in range(0, total_batches):
    with timing():
        ...
        db.session.query(Faktura) \
            ...
            .update({Faktura.customer_invoice_view: enums.CustomerInvoiceView.REGULAR_INVOICES.value})
