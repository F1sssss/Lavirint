...
invoice.tip_fakture = db.session.query(m.FakturaTip).get(m.Faktura.TYPE_CORRECTIVE)
invoice.tip_fakture_id = invoice.tip_fakture.id

invoice.lokacija_dokumenta = None

invoice.korigovana_faktura = db.session.query(m.Faktura).get(2201972)
invoice.korigovana_faktura_id = invoice.korigovana_faktura.id
...
faktura_opb.update_invoice_item_from_upa(
    item=invoice_item_1,
    quantity=Decimal(-1),
    price=Decimal(0.21),
