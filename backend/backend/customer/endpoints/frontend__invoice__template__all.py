...
def api__frontend__invoice__template__all(operater, firma):
    invoice_filters = InvoiceFilterData.load_from_form_dict(bottle.request.query)

    base_filter = InvoiceFilterData()
    total_items = faktura_opb.get_invoice_filter_query(firma.id, operater.naplatni_uredjaj_id, base_filter, enums.CustomerInvoiceView.REGULAR_INVOICE_TEMPLATES).count()

    query = faktura_opb.get_invoice_filter_query(firma.id, operater.naplatni_uredjaj_id, invoice_filters, enums.CustomerInvoiceView.REGULAR_INVOICE_TEMPLATES)
