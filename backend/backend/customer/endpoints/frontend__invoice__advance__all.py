...
def api__frontend__invoice__advance__all(operater, firma):
    invoice_filters = InvoiceFilterData.load_from_form_dict(bottle.request.query)

    f1 = InvoiceFilterData()
    total_items = faktura_opb.get_invoice_filter_query(firma.id, operater.naplatni_uredjaj_id, f1, enums.CustomerInvoiceView.ADVANCE_INVOICES).count()

    query = faktura_opb.get_invoice_filter_query(firma.id, operater.naplatni_uredjaj_id, invoice_filters, enums.CustomerInvoiceView.ADVANCE_INVOICES)
