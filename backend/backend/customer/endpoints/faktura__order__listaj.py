...
def api__faktura__order__listaj(operater, firma):
    filter_data = InvoiceFilterData.load_from_form_dict(bottle.request.query)

    query = faktura_opb.get_order_filter_query(firma.id, operater.naplatni_uredjaj_id, filter_data, enums.CustomerInvoiceView.REGULAR_INVOICES)
