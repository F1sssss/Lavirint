...
def api__frontend__order__all(operater, firma):
    base_filter = InvoiceFilterData()
    total_items = faktura_opb.get_order_filter_query(firma.id, operater.naplatni_uredjaj_id, base_filter, enums.CustomerInvoiceView.REGULAR_INVOICES).count()

    invoice_filters = InvoiceFilterData.load_from_form_dict(bottle.request.query)
    query = faktura_opb.get_order_filter_query(firma.id, operater.naplatni_uredjaj_id, invoice_filters, enums.CustomerInvoiceView.REGULAR_INVOICES)
