...
def controller(agent_user: AgentUser):
    ...
    firma_opb.update_taxpayer_status_by_id(post_data['companyId'], post_data['newStatus'])

    return json.dumps(
        serialize_response(error_message=None, is_taxpayer=post_data['newStatus']),
...
def serialize_response(error_message: Optional[str], is_taxpayer: bool):
    return {
        'error_message': error_message,
        'newStatus': is_taxpayer
    }
